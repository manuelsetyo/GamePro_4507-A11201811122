...
public class cameraFollow : MonoBehaviour {
	...
	void Update () 
	{
		if (target) {
			bool yyy = true;
			bool upp = true;
			point = c.WorldToViewportPoint (new Vector3 (target.position.x, target.position.y, target.position.z));
	...
	public void targerBaru() {
		player = GameObject.Find ("player").GetComponent <Transform> ();
		target = GameObject.Find ("playerPesawat").GetComponent<Transform> ();
	}
}